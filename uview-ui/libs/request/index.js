...
class Request {
	...
	request(options = {}) {
		...
		//如果开启了加密
		if (this.config.encryption && options.params != '') {
			let data = {
				data: authcode(JSON.stringify(options.data), this.config.solt, 'jiami')
			}
			options.data = data
		}
		...
		return new Promise((resolve, reject) => {
			options.complete = (response) => {
				...
				// 	response.data.data = authcode(response.data.data, this.config.solt, 'jiemi')
	...
	constructor() {
		this.config = {
			...
			encryption: false, //是否开启加密
			solt: 'absfds1231##@', //加密的密码,
			authentication: true, //是否开启token鉴权,
			debug: true,
			filter: [] //过滤掉的API不执行拦截部分
		}
